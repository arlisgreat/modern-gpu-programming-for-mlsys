...
| `cp.async.bulk.tensor` | copy 降为 TMA |
...
- `tcgen05_alloc` 和 `tcgen05_dealloc` 有一个 warp guard，但没有 lane guard；issuing warp 的所有 lanes 参与。
...
| 线索 | 可能的区域 | 首先检查 |
|---|---|---|
| 生成的 CUDA 没有 `cp.async.bulk.tensor` | copy 没有降低到 TMA | 检查 `dispatch="tma"`、目标能力和操作数 layout |
