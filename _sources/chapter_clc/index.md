...
在引擎盖下，这种形状直接遵循 CLC 正在做的事情。硬件没有从软件队列中分配抽象任务。它正在取消尚未发布的 cluster 发布。因此，成功的响应包含真实的 cluster 坐标。响应失败仅仅意味着启动队列已耗尽。
...
这种差异在发布临近 epilogue 时最为重要。在静态计划中，剩余的工作可能不会均匀分配。一些 SMs 可能会用完分配的 tile，而其他一些则还剩下几个 tile。对于 CLC，提前完成的 cluster 请求另一个待处理的 cluster 坐标。只要启动队列中还有剩余工作，早期完成者就会不断拉出更多的 tile。
