...
这是从头到尾介绍单个 GEMM 优化路径的三章中的第一章。在本例中，我们构建了一个正确的平铺 kernel 并就此停止。下一章 ({ref}`chap_gemm_async`) 将 thread copy 替换为 TMA，并通过 pipeline 将数据移动与计算重叠，而 {ref}`chap_gemm_advanced` 更进一步，使用 warp specialization 和 CTA clusters。每一章都建立在前一章的基础上，因此 kernels 会积累功能而不是重新开始。
...
> - 调度：同步 `Tx.copy` 承载 load，`tcgen05` 运行 MMA。
...
2. **load**：所有 128 个 threads 协作将 A 和 B tile 从 GMEM copy 到 SMEM（同步 `Tx.copy`）
3. **compute**：单个 elected thread 发出 `Tx.gemm_async` + `tcgen05.commit`；所有 threads 等待 mbarrier
...
因为这里只有一个 tile（M=N=128，K=64），所以 copy 整个 A 和 B 就是整个 load。`Tx.cta.copy(...)` 使 CTA 在该 copy 上进行协作，每个 thread 负责自己的数据片。接下来的 `T.cuda.cta_sync()` 执行双重任务：它等待每个 thread 完成并发布其共享内存写入，以便当 MMA 稍后读取 `Asmem` 和 `Bsmem` 时，它会看到完整的 tile 而不是半填充的缓冲区。这个 thread-driven copy 也是我们要替换的第一个东西；下一章（{ref}`chap_gemm_async`）将其替换为 TMA。
